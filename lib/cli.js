...
exports.interpret = function (argv) {
    ...
    program
        ...
        .option('-q, --quiet', 'less verbose reporter output')
        .option('-m, --mode [' + Object.keys(reporters).join('|') + ']', 'output mode', 'console')
        .option('-x, --exclude <file1,dir1,file2...>', 'files/directories to exclude', function list(val) {
            return val.split(',');
        }, [])
        .option('-p, --parser [' + Object.keys(parsers).join('|') + ']', 'lint parser to use', 'jshint');
    
    // TODO: custom parse options
    // node runforcover? enforce finished event is always emitted from reporter.
    ...
    var reporter = reporterFactory(program.mode, {quiet: program.quiet});
    var parser = parserFactory(program.parser);
    var options = {rootDirectory: program.directory, parser: parser, exclude: program.exclude};
    
    // When the reporter is finished, exit using the right status code.
    reporter.on('finished', function () {
        if (this.hasErrors()) {
    ...
    fileUtils.getHintErrors(options, function (err, hintErrors) {
        ...
        reporter.setErrors(hintErrors).generate();       
    });    
};
